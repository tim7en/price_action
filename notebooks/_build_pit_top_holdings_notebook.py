...
def build_notebook() -> nbf.NotebookNode:
    ...
    cells.append(
        md(
            ...
            "- Removing the full-sample VIX3M backfill (see §2) materially lowered history Sharpe across all candidates "
            "and reduced the number of regime-change periods detected on the 2025+ holdout — direct evidence the prior "
            "VIX3M backfill was injecting future information into both the model and the regime classifier.\n"
            "- Live headline numbers (history winner, holdout outcome, oracle gap) are read from "
            "`sector_ml_pit_top5_holdings_best_strategy_assessment.json` in §11 rather than hardcoded here, so the "
            "narrative stays consistent with whatever the most recent run produced.\n"
            "\n"
    ...
    cells.append(md("## 13. Conclusions and known caveats\n"
                    ...
                    "**What we found.**\n"
                    "\n"
                    "- The PIT runner now reads the split OOS signal exports and produces a real train-to-holdout "
                    "assessment (`sector_ml_pit_top5_holdings_leaderboard.csv`, "
                    "`sector_ml_pit_top5_holdings_best_strategy_assessment.json`).\n"
                    "- After patching the macro VIX3M backfill (causal expanding regression instead of full-sample OLS), "
                    "history Sharpe across all four candidates fell substantially and the regime classifier produces "
                    "fewer regime-change periods on the 2025+ holdout. This is direct evidence the prior pipeline was "
                    "leaking future volatility information into both the model and the regime labels.\n"
                    "- The selection-vs-oracle gap in §11 is the right artifact to monitor — it shows whether the "
                    "history-ranked winner is at least competitive on holdout, not whether it is the ex-post best.\n"
                    "\n"
                    "**Forward-bias status.**\n"
                    "\n"
                    ...
                    "- Macro pipeline: full-sample VIX3M backfill and `bfill()` on spot VIX removed; both replaced with "
                    "causal equivalents (expanding regression and `combine_first(VXO)` only). Verified live in §2.\n"
                    "\n"
                    "**Known caveats.**\n"
                    "\n"
                    "- `ATVI`, `PXD`, `RTN`, `UTX` are skipped — no local price history (delisted/renamed). The runner "
                    "reports them in the missing-symbols log but does not crash.\n"
                    "- The holdout window is short — `regime_change` may produce as few as 1 period after the fix. Treat "
                    "the holdout as a falsification check, not a precision estimate. If you want stable holdout point "
                    "estimates, restrict the candidate set to `ml_5bar` and use `regime_change` only as a diagnostic.\n"
                    "- 10% stop is end-of-day; intraday breaches that recover by close are not modeled.\n"
                    "- Pre-2006 macro rows are dropped at training time because VIX3M is genuinely unknown before then; "
                    "this is the correct causal behavior, not a regression. If you want longer history at training time, "
                    "drop VIX3M from `MODEL_MACRO_CONTEXT_COLUMNS` rather than re-introducing the leaky backfill.\n"))

...
def build_notebook() -> nbf.NotebookNode:
    ...
    cells.append(
        code(
            ...
            "\n"
            "clip_meta = pd.DataFrame(cleaning['outlier_controls']).T\n"
            "display(clip_meta)"
        )
    )
    ...
    cells.append(
        code(
            ...
            "plot_frame = market_structure.loc[market_structure['sector'].isin(top_sectors)].copy()\n"
            "fig, axes = plt.subplots(1, 2, figsize=(14, 4.5))\n"
            "sns.lineplot(data=plot_frame, x='quarter_end_date', y='market_cap_share', hue='sector', ax=axes[0])\n"
            "axes[0].set_title('Market-cap share by sector')\n"
            "axes[0].legend(frameon=False, loc='upper left')\n"
            "sns.lineplot(data=plot_frame, x='quarter_end_date', y='turnover_proxy', hue='sector', ax=axes[1])\n"
            "axes[1].set_title('Turnover proxy by sector')\n"
    ...
    cells.append(
        code(
            ...
            "sns.barplot(data=val, x='model_label', y='roc_auc', ax=axes[0], palette='Blues_d')\n"
            "axes[0].axhline(0.5, color='#e63946', linestyle='--', linewidth=1.0)\n"
            "axes[0].set_title('Validation ROC AUC by model')\n"
            "axes[0].tick_params(axis='x', rotation=25)\n"
            "sns.barplot(data=hold, x='model_label', y='roc_auc', ax=axes[1], palette='Greens_d')\n"
